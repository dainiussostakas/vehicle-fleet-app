<template>
  <main class="p-4 md:ml-64 h-auto pt-20">
    <VehicleBrandAddDialog v-model:visible="addDialogVisible"
...
import TableLazyData from "@/views/vehicle-brand/TableLazyData";
import {ref} from "vue";
import {VehicleBrandService} from "@/service/VehicleBrandService";
import VehicleBrandAddDialog from "@/views/vehicle-brand/VehicleBrandAddDialog.vue";
...
export default {
  components: {VehicleBrandAddDialog},
  emits: [],
  data() {
    let filters = {
  ...
  methods: {
    loadLazyData() {
      ...
      vehicleBrandService.getList(params).then((items: JsonApiResponse) => {
        this.items = [];
